    ...
    <v-layout >
        <v-flex>
            <v-toolbar flat color="white">
                <v-toolbar-title>Caja</v-toolbar-title>
                ...
                <v-dialog v-model="dialog" max-width="500px">
                    <template v-slot:activator="{ on }">
                        <v-btn color="primary" dark class="mb-2" v-on="on">Apertura</v-btn>
                    </template>
                    <v-card>
                        <v-card-title>
                            <span class="headline">Apertura Caja</span>
                        </v-card-title>
                        <v-card-text>
                            <v-container grid-list-md>
                                <v-layout wrap>
                                    <v-flex xs12 sm12 md12>
                                        <v-text-field v-model="monto" type="number" label="Monto"></v-text-field>
                                    </v-flex>
                                    <v-flex xs12 sm12 md12>
                ...
                <v-dialog v-model="dialog" max-width="200px">
                    <template v-slot:activator="{ on }">
                        <v-btn color="warning" dark class="mb-2" @click.native="redirectToAddView()">Cierre Caja</v-btn>
                    </template>
                </v-dialog>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      listar () {
        this.getCajaListFromService(
          this.pagination.currentPage,
          this.pagination.perPage,
        ).then(res => {
          this.cajas = Object.assign([], res.data.results)
        })
      },
      ...
      redirectToAddView () {
        this.$router.push({ name: 'movimiento_caja'})
      },
